...
import { ArrowRight, Calendar, Megaphone, Users, Palette, BookOpen, Info, FileText, Trophy, Sparkles } from 'lucide-react';
...
export const GSAPBentoGrid: React.FC = () => {
  const navigate = useNavigate();
  const isMobile = useIsMobile();
  const gridRef = useRef<HTMLDivElement>(null);
  const { staggerAnimation, slideIn, scaleIn, parallaxScroll } = useGSAP();
  ...
  useEffect(() => {
    ...
    // Initial setup
    slideIn(".bento-header", 'down', { duration: 0.8, scrollTrigger: true });
    
    // Stagger bento cards
    staggerAnimation(".bento-card", 'scaleIn', {
      stagger: 0.1,
      duration: 0.6,
      scrollTrigger: true,
      start: "top 90%"
    ...
    const patterns = gridRef.current.querySelectorAll('.bento-pattern');
    patterns.forEach(pattern => {
      parallaxScroll(pattern, 0.3);
    });

  }, [slideIn, staggerAnimation, scaleIn, parallaxScroll]);
  ...
  const getPatternSvg = (pattern: string, id: string, tileTextColor: string = "text-white") => {
    ...
    switch (pattern) {
      case "dots":
        return (
          <svg className={`bento-pattern absolute inset-0 w-full h-full ${baseOpacity} ${colorClass}`} viewBox="0 0 100 100">
      ...
      case "hexagon":
        return (
          <svg className={`bento-pattern absolute inset-0 w-full h-full opacity-15 ${colorClass}`} viewBox="0 0 100 100">
  ...
  return (
    <div ref={gridRef}>
      <div className="bento-header mb-6 md:mb-8">
        <div className="text-center space-y-2 md:space-y-4">
          <div className="flex flex-col items-center leading-tight">
            ...
            <h1 className="text-3xl md:text-5xl lg:text-6xl xl:text-7xl font-sans font-bold bg-gradient-to-r from-amethyst via-turquoise to-coral bg-clip-text text-transparent">
              OUR CREATIVITY
            </h1>
          </div>

          <div className="flex items-center justify-center gap-2 text-xs md:text-sm text-foreground/60 dark:text-foreground-dark/60">
              ...
              <div className="relative z-10 p-4 md:p-5 lg:p-6 h-full flex flex-col justify-center text-center">
                <div className="space-y-4 md:space-y-5">
                  ...
                  <div className="space-y-2 md:space-y-3">
                    <h3 className={`text-lg md:text-xl lg:text-2xl font-sans font-bold ${tile.textColor || "text-foreground"} leading-tight drop-shadow-lg`}>
                      {tile.title}
                    </h3>
                ...
                <div className="flex items-center justify-between mt-6 md:mt-8 pt-3 md:pt-4 border-t border-white/20">
                  ...
                  <div className={`flex items-center gap-1.5 md:gap-2 ${tile.textColor || "text-foreground"} font-semibold transition-all duration-300 font-sans group-hover:translate-x-1`}>
                    <span className="text-sm md:text-base">Jelajahi</span>
                    <ArrowRight className="w-4 h-4 md:w-5 md:h-5 drop-shadow-sm transition-transform duration-300" />
                  </div>
                </div>
              </div>
