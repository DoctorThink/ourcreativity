...
const AdminActivityLog = () => {
  ...
  const getActionBadge = (action: string) => {
    switch (action) {
      ...
      case 'updated':
        return <Badge variant="outline" className="bg-foreground/5 text-azure border-azure/30">Diperbarui</Badge>;
      default:
  ...
  const getCategoryBadge = (category: string) => {
    switch (category) {
      ...
      case 'video':
        return <Badge variant="secondary" className="bg-azure/10 text-azure border-azure/30">Video</Badge>;
      case 'meme':
      ...
      <CardContent>
        <div className="space-y-4">
          {isLoading ? (
            ...
            logs.map((log) => (
              <div key={log.id} className="flex items-start space-x-4 pb-4 border-b border-foreground/5 last:border-0">
